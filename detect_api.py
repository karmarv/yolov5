...
# Init model
device, model, class_names = init_model(weights="/home/rahul/workspace/mmm/sura/detector/yolov5/yolov5s.pt")
...
def format_detect_result(path, img_og, img, preds, names):
    formatted_result, s = ["{}".format(path)], ""
    ...
    for i, det in enumerate(preds):
        ...
        if det is not None and len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                label = '%.6f' % (conf)     
                # class_submission_id, x_min, y_min, x_max, y_max, confidence
                out_str = "{0} {1} {2} {3} {4} {5}".format(int(cls)+1, int(xmin), int(ymin), int(xmax), int(ymax), label)
                score_dict[label] = out_str
            for key in sorted(score_dict.keys(), reverse=True):
                #print(key)
                formatted_result.append(score_dict[key])
    print('[Result] - %s' % (s))
